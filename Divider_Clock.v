...
module Divider_Clock #(
        ...
        output  reg clkout_music = 1,   // ? 新增：音樂節拍用
    ...
    parameter Divider_Counter_100  = 1_000_000;
    parameter Divider_Counter_10   = 10_000_000;
    parameter Divider_Counter_1    = 100_000_000;

    // ? 新增：音樂節拍，先抓 4Hz 試聽
    parameter Divider_Counter_MUSIC = 25_000_000;
    ...
    always@(posedge clkin or negedge rst_n)begin
        if(!rst_n) begin
            ...
            // ? 音樂 4Hz
            if(Counter_music == (Divider_Counter_MUSIC - 1))
                Counter_music <= 0;
            else
    ...
    always@(posedge clkin or negedge rst_n) begin
        if(!rst_n) begin
            ...
            // ? 音樂 4Hz
            if(Counter_music < Divider_Counter_MUSIC/2)
                clkout_music <= 1'b0;
            else
                clkout_music <= 1'b1;
